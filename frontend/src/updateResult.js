...
const UpdateResult = () => {
  ...
  useEffect(() => {
    const fetchResult = async () => {
      try {
        const res = await axios.get(`https://result-in-sand.vercel.app/api/result/${id}`);
        setResult(res.data);
      } catch (err) {
        console.error(err);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.put(`https://result-in-sand.vercel.app/api/update/result/${id}`, result);
